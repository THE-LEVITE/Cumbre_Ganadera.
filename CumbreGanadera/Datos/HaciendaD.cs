...
namespace CumbreGanadera.Datos
{
    ...
    {
        public List<Hacienda> MTDueñoHacienda(int IdDueño)
        {
        ...
        public List<Hacienda> MTListarTodasHaciendas()
        {
            using (SqlConnection cn = ConexionBD.MtAbrirConexion())
            {
                cn.Open();

                string consulta = "select h.Id, h.FotoHacienda, h.NombreHacienda, h.TipoProduccion, c.NombreCiudad, d.Departamento from Hacienda h join Ciudad c on h.IdCiudad = c.Id join Departamento d on c.IdDepartamento = d.Id";
